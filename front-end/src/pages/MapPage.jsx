...
// --- ICONS ---
// Fierce Target Icon for Location Button
const TargetIcon = () => (
...
const mapStyles = `
  ...
  /* USER LOCATION PULSING CIRCLE (Radar Effect) */
  .user-pulse-marker {
    position: relative;
    display: flex;
    align-items: center;
...
function SearchOverlay({ points, onSelectResult }) {
    ...
    const results = useMemo(() => {
        ...
        return points.filter(p => 
            (getChargerTitle(p) + getId(p)).toLowerCase().includes(query.toLowerCase())
        ).slice(0, 5);
    }, [query, points]);
            ...
            <Card className="bg-zinc-900/95 border border-zinc-700 shadow-2xl backdrop-blur-md">
                <CardBody className="p-2">
                    <Input 
                        ...
                        classNames={{ inputWrapper: "bg-zinc-800", input: "text-white" }}
                    />
                </CardBody>
            </Card>
            {showResults && results.length > 0 && (
                <Card className="mt-2 bg-zinc-900/95 border border-zinc-700 backdrop-blur-md">
                    <div className="flex flex-col">
                        {results.map((p) => (
                            <button key={getId(p)} onClick={() => { onSelectResult(p); setShowResults(false); }} className="p-3 hover:bg-zinc-800 text-left text-white border-b border-zinc-800 text-sm">
                                {getChargerTitle(p)}
                            </button>
                        ))}
                    </div>
...
const SmartMarker = ({ point, selectedId, navigate, userLocation, activeReservationId }) => {
    ...
    const distance = userLocation 
        ? getDistanceFromLatLonInM(userLocation.lat, userLocation.lon, parseFloat(point.lat), parseFloat(point.lon))
        : Infinity;

    const isReservedByMe = String(activeReservationId) === String(myId);
    const canCharge = distance < 500 || isReservedByMe;

    useEffect(() => {
        if (selectedId === String(myId) && markerRef.current) {
            setTimeout(() => markerRef.current.openPopup(), 500);
        }
    }, [selectedId, myId]);

    const statusColor = (point.status||'').toUpperCase() === 'AVAILABLE' ? "status-available" : 
                        (point.status||'').toUpperCase() === 'RESERVED' ? "status-reserved" : "status-charging";
    
    const icon = L.divIcon({
        className: 'custom-div-icon', 
        html: `<div class="led-marker ${statusColor}"><div class="led-core"></div></div>`,
        iconSize: [24, 24]
    });
                ...
                <div className="p-5 bg-black/90 border border-zinc-800 rounded-2xl text-white min-w-[240px] shadow-[0_0_30px_rgba(0,0,0,0.8)] backdrop-blur-xl">
                    <div className="mb-4 border-b border-zinc-800 pb-3">
                        ...
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] text-zinc-500 font-mono">ID: {myId}</span>
                            <Chip size="sm" variant="dot" color={(point.status||'').toUpperCase() === 'AVAILABLE' ? "success" : "warning"} className="h-5 text-[10px] border-0">
                                {point.status}
                            </Chip>
                        </div>
                        {userLocation && (
                    ...
                    <div className="flex flex-col items-center justify-center bg-zinc-900 rounded-xl p-3 mb-4 border border-zinc-800">
                        <span className="text-[10px] text-zinc-400 uppercase tracking-widest font-bold">Max Power</span>
                        <div className="flex items-baseline gap-1">
                            <span className="text-3xl font-black text-white tracking-tighter">{point.cap}</span>
                            <span className="text-sm font-bold text-zinc-500">kW</span>
                        </div>
                    </div>
                    
                    {(point.status || '').toUpperCase() === 'AVAILABLE' && (
                        <div className="flex gap-2">
                            <Button onPress={() => navigate(`/reserve/${myId}`, { state: { fromMap: true } })} size="sm" variant="flat" className={`bg-zinc-800 font-bold text-white ${canCharge ? 'flex-1' : 'w-full'}`}>Reserve</Button>
                            {canCharge && <Button onPress={() => navigate(`/charging/${myId}`)} size="sm" className="flex-1 bg-green-500 font-bold text-black shadow-[0_0_15px_rgba(34,197,94,0.4)] animate-pulse">Charge ⚡</Button>}
                        </div>
                    )}
                    {(point.status || '').toUpperCase() === 'RESERVED' && (
                        isReservedByMe ? (
                            <Button onPress={() => navigate(`/charging/${myId}`)} size="sm" className="w-full bg-orange-500 font-bold text-white animate-pulse">Start Session</Button>
                        ) : (
                            <Button disabled size="sm" className="w-full bg-zinc-800 text-zinc-500 font-bold">Reserved</Button>
                        )
                    )}
                     {(point.status || '').toUpperCase() === 'CHARGING' && (
                        <Button onPress={() => navigate(`/charging/${myId}`)} size="sm" className="w-full bg-purple-600 font-bold text-white">Monitor</Button>
                    )}
                </div>
            </Popup>
        </Marker>
...
export default function MapPage() {
  ...
  useEffect(() => {
      const fetchReservation = async () => {
          try {
              ...
              if (res.data && res.data.pointid) setActiveReservationId(res.data.pointid);
              else setActiveReservationId(null);
  ...
  useEffect(() => {
    if (location.state && location.state.targetId && points.length > 0) {
        ...
        if (point && mapRef.current) {
            ...
            setTimeout(() => setSelectedId(targetId), 600);
        }
    }
  }, [location, points]);
  ...
  return (
    <div className="h-full w-full relative bg-zinc-950 overflow-hidden">
      ...
      {/* FIERCE LOCATION BUTTON */}
      <div className="absolute bottom-6 right-4 z-[900]">
          <Button 
            isIconOnly 
            onPress={() => userLocation && mapRef.current.flyTo([userLocation.lat, userLocation.lon], 16)} 
            className="bg-zinc-900 border border-zinc-700 text-white rounded-full w-14 h-14 shadow-2xl hover:scale-110 hover:border-white transition-all"
          >
            <TargetIcon />
          </Button>
      </div>
      ...
      <MapContainer ref={mapRef} center={[37.9838, 23.7275]} zoom={13} style={{ height: '100%', width: '100%' }} zoomControl={false}>
        ...
        <MarkerClusterGroup 
            ...
            maxClusterRadius={45} 
            iconCreateFunction={(cluster) => L.divIcon({ html: `<div class="custom-cluster-marker" style="width:40px;height:40px;">${cluster.getChildCount()}</div>` })}
        >
            {points.map((point) => (
                <SmartMarker 
                    key={getId(point)} 
        ...
        {userLocation && (
            <Marker 
                ...
                icon={L.divIcon({ 
                    ...
                    iconSize: [40, 40] // Important so it doesn't clip
                })} 
            />
        )}
      </MapContainer>
