...
export default function ReservationPage() {
  ...
  useEffect(() => {
    const fetchNearby = async () => {
        ...
        try {
            ...
            setNearbyPoints(available.slice(0, 10)); // Top 10 closest
  ...
  return (
    <div className="min-h-screen w-full bg-black flex justify-center p-4 pt-10 text-white overflow-y-auto">
      <div className="w-full max-w-md flex flex-col gap-6 pb-20">
        
        <div className="flex items-center justify-between"><span className="text-2xl font-bold tracking-tight">Reservations</span>{viewMode === 'active' && <Chip size="sm" color="warning" variant="dot">Active</Chip>}</div>
        ...
        {viewMode === 'active' && reservationData && (
            <Card className="w-full bg-zinc-900 border border-green-500/50 p-6 shadow-[0_0_40px_rgba(34,197,94,0.1)]">
        ...
        {viewMode === 'standby' && (
            <Card className="w-full bg-zinc-900/50 border border-zinc-800 border-dashed p-10 flex flex-col items-center text-center">
        ...
        {/* --- NEARBY CHARGERS (SCROLLABLE) --- */}
        <div className="w-full max-w-md mt-12 mb-10 border-t border-zinc-800 pt-6">
            <h3 className="text-lg font-bold text-zinc-400 mb-4 uppercase tracking-widest">Closest Chargers</h3>
                        ...
                        <CardBody className="flex flex-row justify-between items-center p-4">
                            <div className="flex flex-col items-start text-left">
                                ...
                                <div className="flex gap-2 mt-1">
                                    ...
                                    <span className="text-xs text-blue-400 bg-blue-900/30 border border-blue-500/30 px-2 py-0.5 rounded font-mono">
                                        {Math.round(point.distance)}m
                                    </span>
                                </div>
                            </div>
                            <Button size="sm" className="bg-zinc-800 text-white font-bold border border-zinc-700">Book</Button>
                        </CardBody>
                    </Card>
                )) : (<p className="text-zinc-600 italic">Getting location...</p>)}
            </div>
        </div>
